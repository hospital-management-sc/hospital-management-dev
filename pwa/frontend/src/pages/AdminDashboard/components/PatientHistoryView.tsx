        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Nro. Historia Clínica:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.nroHistoria || patient.nroHistoria || 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Nombre Completo:</strong>
              <span style={{ fontSize: '1rem', fontWeight: '500' }}>{historiaCompleta?.apellidosNombres || patient.apellidosNombres || 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Cédula de Identidad:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.ci || patient.ci || 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Tipo de Paciente:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.tipoPaciente || patient.tipoPaciente || 'N/A'}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <span style={{ fontSize: '1rem' }}>{(historiaCompleta?.sexo || patient.sexo) === 'M' ? 'Masculino' : (historiaCompleta?.sexo || patient.sexo) === 'F' ? 'Femenino' : 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Nacionalidad:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.nacionalidad || patient.nacionalidad || 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Lugar de Nacimiento:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.lugarNacimiento || patient.lugarNacimiento || 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Estado de Residencia:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.estado || patient.estado || 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Religión:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.religion || patient.religion || 'N/A'}</span>
            </div>
          </div>
        </div>

        {/* Sección 3: Información de Contacto */}
        <div style={{ marginBottom: '0', paddingTop: '1rem', borderTop: '1px solid var(--border-color)' }}>
          <h4 style={{ 
            fontSize: '0.95rem', 
            fontWeight: '600', 
            color: 'var(--text-secondary)', 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>📞 Teléfono Principal:</strong>
              <span style={{ fontSize: '1rem', fontWeight: '500' }}>{historiaCompleta?.telefono || patient.telefono || 'N/A'}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>🚨 Teléfono de Emergencia:</strong>
              <span style={{ fontSize: '1rem', fontWeight: '500', color: '#ef4444' }}>
                {historiaCompleta?.telefonoEmergencia || patient.telefonoEmergencia || 'No registrado'}
              </span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', gridColumn: '1 / -1' }}>
              <strong style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>🏠 Dirección Completa:</strong>
              <span style={{ fontSize: '1rem' }}>{historiaCompleta?.direccion || patient.direccion || 'N/A'}</span>
            </div>
          </div>
        </div>
      </div>
